...
const composeSummary = () => {
  ...
  let dateOfDayBefore = moment().tz('Europe/Paris').add(-2, 'hour').add(-1, 'day').add(2, 'hour').toISOString()
  // Get timerHistory of the day before
  fetch("https://paymemobile.fr/timerHistory?datetime=" + dateOfDayBefore).then((response) => {
    response.json().then((timerHistory) => {
      console.log(timerHistory)
      timerHistory.forEach((x) => {
        timeSummary.push({
          subsub: x._id,
          duration: x.totalDuration * 60 * 1000,
        })
      })
    }).then(() => {
      // Get completed tasks of the day before
      fetch("https://paymemobile.fr/subsubHistory?datetime=" + dateOfDayBefore).then((response) => {
        response.json().then((subsubHistory) => {
          console.log(subsubHistory)
          subsubHistory.forEach((x) => {
            taskSummary.push({
              sname: x.sname,
              time: moment(x.time).tz('Europe/Paris').format("DD/MM/YYYY HH:mm:ss")
            })
          })
        })
      }).then(() => {
        ...
        let message = "Dear Thinh,\n\nThis is the summary for the day of " + moment().tz('Europe/Paris').startOf('day').format("ddd DD/MM/YYYY") + ". \n\n" + "Work of the day before: \n\n" + timeJoint + "\n" + taskJoint + "\n\n" +
          "Todo(s): \n\n" + xx.todo +
          "\n\n" + "Calendar events:\n\n" + xx.calendar +
          "\n\n" + "Soon to expire tasks:\n\n" + xx.soonExpire +
          "\n\n" + "Thanks for your efforts. Remember: Resilience, Curiosity, Efficiency and Self-reliance are the keys to success.\n\nYours sincerely,\nThe OrdoWallet Team."
