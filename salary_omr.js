...
const preprocess = async () => {
  return new Promise(async (resolve, reject) => {
    ...
    let timeunits = reader.question("Number of time units accumulated: ");
    let stampValue = Number(timeunits) * 1.8;
    ...
    stampRef.set({
      'sn': sn,
      'content': 'Salary for ' + timeunits + ' time units, available from ' + moment().toISOString() + '.',
      'claimed': false,
      'value': stampValue
    });
    resolve();
